...
function Postcard({
  ...
  return (
    <div className="flex flex-col w-[480px] p-8 px-9 gap-8">
      <div className="border rounded-md flex flex-col border-none">
        <div className="flex justify-between items-center py-3">
          <div className="flex items-center justify-start gap-4">
            <img src={`http://localhost:9000/profile/` + profileImage} alt="profpic" className="w-8 h-8 rounded-full" />
            <h3 className="text-2xl text-white font-semibold">@{username}</h3>
          </div>
        </div>
        <img src={`http://localhost:9000/posts/` + image} alt="postimg" className="h-96 " />
